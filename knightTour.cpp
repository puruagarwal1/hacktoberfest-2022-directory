/* Contributor : G. Pala Venki Reddy */

/* knight Tour Problem  Description */
/* 
...
/* 
   The only line has two integers x and y: the knight's starting position on the chessboard.
*/

/* Output Format */

/* 
   Print a grid that shows how the knight moves (according to the example)
...
int main()
{
    ...
    x--; y--;
    int move = 1;
    /* backtrack(grid, x, y, move); */ /* in worst case scenerio, time complexity is O(8^(n^2)) which is too large */ 
    /* so we will use condition that move the knight to cell which is valid and also possesss less moves compared to other valid moves */
